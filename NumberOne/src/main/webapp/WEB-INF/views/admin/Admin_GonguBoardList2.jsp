	                   ...
	                   <tr style="border-bottom: solid #E0E0E0 1px;">
	                      ...
	                      <td>
	                      	<c:choose>
	                      		...
	                      		<c:when test="${notice.nbstate == 3}">
	                      			<button class="btn btn-sm btn-primary" type="button" onclick="showNbstateModal(this,'${notice.nbcode }')">완료</button>
	                      		</c:when>
	                      		<c:otherwise>
	                      ...
	                      <td id="fixBtn_${notice.nbcode }">
	                      	<c:if test="${notice.nbstate == 1 || notice.nbstate == 3 }">
		                      	<c:choose>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                	...
                	<c:choose>
	                	<c:when test="${btnstate}=='진행'">
		                	<button class="btn btn-success" onclick="updateNbstateEnd()">완료</button>
		                    <button class="btn btn-danger" onclick="updateNbstateDelete()">삭제</button>
		                    <button class="close btn btn-secondary" type="button" data-dismiss="modal">취소</button>
	                	</c:when>
	                	<c:when test="${btnstate}=='삭제'}">
		                	<button class="btn btn-primary" onclick="updateNbstate()">진행</button>
		                    <button class="btn btn-success" onclick="updateNbstateEnd()">완료</button>
		                    <button class="close btn btn-secondary" type="button" data-dismiss="modal">취소</button>
	                    </c:when>
	                    <c:otherwise>
		                	<button class="btn btn-primary" onclick="updateNbstateEnd()">진행</button>
		                    <button class="btn btn-danger" onclick="updateNbstateDelete()">삭제</button>
		                    <button class="close btn btn-secondary" type="button" data-dismiss="modal">취소</button>
	                	</c:otherwise>
                    </c:choose>
	...
	<script type="text/javascript">
		...
		function showNbstateModal(obj, nbcode){
			...
			if (btnObj_stateText == "진행"){
				$("#updateNbstateModalBody").text(nbcode + "번 공구를 완료/삭제 처리하시겠습니까?");
			} else if (btnObj_stateText == "삭제"){
				$("#updateNbstateModalBody").text(nbcode + "번 공구를 진행/완료 처리하시겠습니까?");
			} else {
				$("#updateNbstateModalBody").text(nbcode + "번 공구를 진행/삭제 처리하시겠습니까?");
			}
			$("#nbcode_state").val(nbcode);
			$("#updateNbstateModal").modal("show");
		}
		
		// 공구상태 변경 모달창에서 "삭제" 버튼을 눌렀을 때 상태값 변경하고 상태 버튼 css 변경
		function updateNbstateDelete(){
			console.log("updateNbstateDelete() 실행");
